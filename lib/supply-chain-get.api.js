(function(){ 
  ...
  $('#Order').addEventListener('change', function() { 
    const relation_list = [];
    if ($('#product_relation').checked){relation_list.push('CONTAINS')}
    if ($('#customer_relation').checked){relation_list.push('PLACED')}
    if ($('#location_relation').checked){relation_list.push('DELIVERED_TO')}
    if ($('#bottleneck_relation').checked){relation_list.push('HAS_BOTTLENECK')}
    if ($('#profit_relation').checked){relation_list.push('HAS_PROFIT')}

    // Display the order node first, in case the next query has null result
    retrieve($('#Order').value, '_', 'order', 'query');  
    retrieve($('#Order').value, relation_list, 'order_relationList', 'query');
  });
  ...
  $('#Product').addEventListener('change', function() { 
    ...
    retrieve($('#Product').value, relation_list, 'product_relationList', 'query');
  });
  ...
  $('#hot_product_regions').addEventListener('click', function() { 
    retrieve('Hot Product Analysis', '_', 'hot_product_regions', 'query') 
  });

  $('#hot_product_regions_table').addEventListener('click', function() {
    retrieve2('Hot Product Analysis Table', '_', 'hot_product_regions_table', 'query2')
  ...
  $('#high_profit_by_product_btn').addEventListener('click', function() { 
    ...
    const profitThreshold = $('#profit_threshold').value || "100";
    ...
    fetch(API_string)
      ...
      .then(json_value => { 
        ...
        for (let x in json_value) {
          ...
          for (let y in json_value[x]._fields) {
            let fieldValue = json_value[x]._fields[y];
            let displayValue = "";
            
            if (fieldValue === null || fieldValue === undefined) {
              displayValue = "";
            } else if (typeof fieldValue === 'object') {
              // Handle Neo4j integers (objects with .low property)
              if (fieldValue.low !== undefined) {
                displayValue = fieldValue.low;
              } 
              // Handle date objects from Neo4j
              else if (fieldValue.year && fieldValue.month && fieldValue.day) {
                displayValue = `${fieldValue.year.low}-${fieldValue.month.low}-${fieldValue.day.low}`;
              }
              // Try to extract other object properties
              else {
                try {
                  displayValue = JSON.stringify(fieldValue).replace(/[{}"]/g, '').replace(/,/g, ', ');
                } catch (e) {
                  displayValue = "[Complex Object]";
                }
              }
            } else {
              // For numbers, format with up to 2 decimal places
  ...
  let cy_addListener = cy => {    // Create function to attach actions to clicks on graph nodes and edges
    ...
    cy.on('tap', 'node', function(event){
      ...
      // Safely extract and format the ID - handle objects properly
      let nodeId = node.data()['id2'];
      if (typeof nodeId === 'object' && nodeId !== null) {
        // If it's an object, extract values for display
        if (nodeId.low !== undefined) {
          nodeId = nodeId.low; // Extract Neo4j integer value
        } else {
          // Try to convert object to meaningful string
          const idParts = [];
          for (const key in nodeId) {
            if (nodeId.hasOwnProperty(key)) {
              idParts.push(`${key}:${nodeId[key]}`);
            }
          }
          nodeId = idParts.length > 0 ? idParts.join(', ') : 'Unknown ID';
        }
      }
      
      // html text to display in panel. Start with ID2 field
      let txt = '<p><b>ID: ' + nodeId + '</b></p>';  
      txt = txt.replace(/_/g, ' ');   // replace _ with space
    
      // Display the node type
      txt += '<p><b>Type: ' + node.data()['supertype'] + '</b></p>';
      
      // Add profit information for Orders and ProductReferences
      if (node.data()['supertype'] === 'Order' || node.data()['supertype'] === 'Product') {
        const profit = node.data()['profit'];
        if (profit !== undefined && profit !== null) {
          txt += '<p><b>Profit: ' + (typeof profit === 'number' ? profit.toFixed(2) : profit) + '</b></p>';
        }
      }
        
      // Display all other properties
      for (let x in node.data()) {
        // Skip fields already displayed or handled specially
        if (['id', 'id2', 'supertype', 'type', 'label'].includes(x)) {
          continue;
        }
        
        // Format special date fields
        if (['orderDate', 'shippedDate'].includes(x)) {
          txt += '<p><em>' + x + '</em>: ' + node.data()[x] + '</p>';
          continue;
        }
        
        // Format risk metrics
        if (['delayRisk', 'geographicRisk', 'marketRisk'].includes(x)) {
          const riskValue = node.data()[x];
          if (riskValue !== undefined && riskValue !== null && riskValue !== '') {
            txt += '<p><em>' + x + '</em>: ' + 
              (typeof riskValue === 'number' ? riskValue.toFixed(3) : riskValue) + '</p>';
          }
          continue;
        }
        
        // Format object fields properly
        const value = node.data()[x];
        if (value !== undefined && value !== null && value !== '') {
          ...
          if (typeof value === 'object') {
            ...
            if (value.low !== undefined) {
              displayValue = value.low; // Handle Neo4j integers
            } else {
              try {
                displayValue = JSON.stringify(value);
                // Clean up the stringified output
          ...
          txt += '<p><em>' + x + '</em>: ' + displayValue + '</p>';
        }
      }
      
      // Store ID values safely for buttons
      $('#node-expand').value = nodeId;  // Add node ID2 to button value
